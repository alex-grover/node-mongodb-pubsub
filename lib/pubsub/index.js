...
mongo.Db.prototype.watch = function(type, callback) {
  ...
  var filter = {namespace: {$regex: '^' + this.databaseName, $not: new RegExp('^' + this.databaseName + '\.system')}};
  if (type) {
    filter.type = type;
  }
...
mongo.Collection.prototype.watch = function(type, callback) {
  ...
  var namespace = this.db.databaseName + '.' + this.collectionName;

  var filter = {namespace: {$regex: '^' + namespace}};
  if (type) {
    filter.type = type;
  }
