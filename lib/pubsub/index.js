 ...
 * @api public
 */
mongo.MongoClient.prototype.subscribe = function(channel, options, callback) {
  if (typeof channel !== 'string') {
  ...
  this._db.command(subscribeCommand, function(err, subscription) {
...
/**
 * Polls on all subscriptions on this MongoClient for new messages.
...
mongo.MongoClient.prototype.poll = function() {
  ...
  var self = this;
  this._db.command({poll: this.subscriptionIDs, timeout: this.pollLength}, function(err, res) {
    if (err) {
...
mongo.MongoClient.prototype.unsubscribe = function(subscription, callback) {
  ...
  if (index === -1 || !delete this.subscriptions[subscription.id]) {
    callback(new Error('Subscription passed not associated with this MongoClient'));
  }
...
// Add support for DB events on the DB and Collection objects
mongo.Db = require('./dbevents.js').Db;
mongo.Collection = require('./dbevents.js').Collection;
